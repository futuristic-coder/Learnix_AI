  ...
  const renderMessage = (msg, index) => {
    ...
    return (
      ...
      >
        ...
        <div
          className={`max-w-[70%] p-4 rounded-2xl ${isUser ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-800"}`}
        >
          {isUser ? (
            <p>{msg.content}</p>
          ) : (
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12">
        <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-indigo-50 mb-4">
          <MessageSquare className="h-8 w-8 text-indigo-600" strokeWidth={2} />
        </div>
        <Spinner />
        <p className="text-sm text-slate-600 mt-4">Loading chat history...</p>
      </div>
    );
  }
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        {history.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center py-12">
            <div className="flex h-20 w-20 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-50 to-blue-50 mb-6">
              <MessageSquare className="h-10 w-10 text-indigo-600" strokeWidth={2} />
            </div>
            <h3 className="text-xl font-semibold text-slate-900">Start a conversation</h3>
            <p className="mt-2 text-sm text-slate-500 max-w-md">Ask me anything about your documents.</p>
          </div>
        ) : (
      ...
      <div className="border-t border-slate-200 bg-white p-4">
        <form onSubmit={handleSendMessage} className="flex items-center gap-3">
          <input
            ...
            disabled={loading}
            className="flex-1 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-800 placeholder:text-slate-400 focus:border-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-100 disabled:bg-slate-50 disabled:cursor-not-allowed"
          />
          <button 
            type="submit" 
