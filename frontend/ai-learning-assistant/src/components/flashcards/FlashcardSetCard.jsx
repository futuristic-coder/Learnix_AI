...
const FlashcardSetCard = ({ flashcardSet }) => {
  ...
  return (
    <div onClick={handleStudyNow} className="group bg-white rounded-2xl shadow-sm border border-slate-200 hover:shadow-lg transition-all p-6 cursor-pointer h-full flex flex-col">
      <div className="flex-1">
        <div className="flex items-start gap-3 mb-4">
          <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-indigo-50 group-hover:bg-indigo-100 transition flex-shrink-0">
            <BookOpen className="h-6 w-6 text-indigo-600" strokeWidth={2} />
          </div>
          <div className="flex-1 min-w-0">
            <h3 title={flashcardSet?.documentId?.title} className="text-lg font-semibold text-slate-900 line-clamp-2">
              {flashcardSet?.documentId?.title}
            </h3>
            <p className="text-xs text-slate-500 mt-1">Created {moment(flashcardSet.createdAt).fromNow()}</p>
          </div>
        </div>
