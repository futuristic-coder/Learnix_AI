...
const QuizCard = ({quiz, onDelete}) => {
  ...
  return (
    <div className="relative bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-xl transition-all duration-300 p-6 flex flex-col group">
      <button
        onClick={(e) => {
          e.stopPropagation();
          onDelete(quiz);
        }}
        className="absolute top-4 right-4 p-2 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50 transition-all duration-200 opacity-0 group-hover:opacity-100"
        title="Delete quiz"
      >
      ...
      <div className="space-y-5 flex-1">
        ...
        <div className="pr-8">
          <h3
            title={quiz.title}
            className="text-xl font-bold text-slate-900 line-clamp-2 mb-2"
          >
            {quiz.title ||
