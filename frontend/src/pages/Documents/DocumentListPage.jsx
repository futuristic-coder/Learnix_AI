    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm p-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-semibold text-slate-900 dark:text-slate-100">My Documents</h1>
            <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Manage and organize your documents efficiently.</p>
          </div>
          {documents.length > 0 && (
            <Button onClick={() => setIsUploadModalOpen(true)}>
      ...
      {isUploadModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-xl bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 p-6 relative">
            <button onClick={() => setIsUploadModalOpen(false)} className="absolute right-4 top-4 p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition">
            ...
            <form onSubmit={handleUpload} className="space-y-4">
              <div className="space-y-2">
                ...
                <input
                  ...
                  required
                  className="w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-4 py-2.5 text-sm text-slate-800 dark:text-slate-100 shadow-sm focus:border-indigo-500 dark:focus:border-indigo-400 focus:outline-none focus:ring-4 focus:ring-indigo-100 dark:focus:ring-indigo-900"
                  placeholder="e.g. Biology Chapter 1"
                />
              </div>

              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-700 dark:text-slate-300">PDF files</label>
                <div className="relative border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl p-6 text-center hover:border-indigo-400 dark:hover:border-indigo-500 transition">
                  <input
                    id="file-upload"
                    type="file"
                    accept=".pdf"
                  ...
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p>
                      {uploadFile ? (
                        <span className="dark:text-slate-300">{uploadFile.name}</span>
                      ) : (
                        <>
                          <span className="text-sm font-medium text-indigo-600 dark:text-indigo-400">Click to upload</span>{" "}
                          <span className="dark:text-slate-400">or drag and drop</span>
                        </>
                      )}
                    </p>
